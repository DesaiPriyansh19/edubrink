...
const CollegeCard = ({ data, loading }) => {
  ...
  if (loading && (!data || data.length === 0)) {
    return (
      <div
        className={`${
          path === "/en/searchresults/university" ? "grid grid-cols-1 sm:grid-cols-2 gap-4" : "flex gap-2"
        }`}
      >
          ...
          return (
            <div
              ...
              className={`relative mt-6 border rounded-xl shadow-md bg-white ${
                path === `/${language}/searchresults/university` ? "" : "max-w-full sm:max-w-md md:max-w-lg"
              } `}
            >
              <div className="p-4 sm:p-6">
              ...
              <div className="grid gap-6 px-3 grid-cols-2 mb-6 mt-4">
                ...
                <button
                  onClick={() => {
                    handleNavigate(language === "ar" ? university.uniName.ar : university.uniName.en)
                  }}
...
function Univrsiry({ loading: initialLoading, filteredData: initialData, countryIds }) {
    ...
    try {
      ...
      if (response.data.data && response.data.data.length > 0) {
        setUniversities((prevUniversities) => {
          ...
          const newUniversities = response.data.data.filter((uni) => !existingIds.has(uni._id))
  ...
  }, [fetchTrigger, loadingMore, hasMore, isSearchResultsPath, initialFetch, fetchMoreUniversities])
        ...
        {!loading && universities?.length === 0 && !isSearchResultsPath && (
          <div className="col-span-full text-center py-8">
            <p className="text-lg text-gray-500">
              {t("noUniversitiesFound") || "No univeristies found. Try adjusting your filters."}
            </p>
          </div>
        )}
      </div>
