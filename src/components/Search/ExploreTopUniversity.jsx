...
function ExploreTopUniversity({ language }) {
  ...
  const [initialLoading, setInitialLoading] = useState(universityCache.data.length === 0)
  ...
  const [initialFetch, setInitialFetch] = useState(universityCache.data.length === 0)
  ...
  const fetchUniversities = useCallback(async () => {
    ...
    try {
      ...
      console.log("Fetching universities with lastId:", lastId, "initialFetch:", initialFetch)
      ...
      setUniversities((prevUniversities) => {
        ...
        if (initialFetch) {
          ...
          const existingIds = new Map(prevUniversities.map((uni) => [uni._id, true]))
          // Filter out duplicates
          const newUniversities = result.data.filter((uni) => !existingIds.has(uni._id))
  ...
  if (initialLoading) {
    return <BouncingBarsLoader type="gradient" message={t("Loading Universities...")} />
  }

  return (
    <div dir={language === "ar" ? "rtl" : "ltr"} className="p-4">
      {/* Heading */}
      <h1 className="text-5xl text-center max-w-2xl mx-auto font-semibold mb-2">{t("explore_university.title")}</h1>
      ...
      {/* Dynamic Buttons */}
      <h3 className="text-4xl font-semibold mb-11">{t("explore_university.top_university")}</h3>
                        ...
                        university?.uniCountry?.countryCode ? (
                          <div className="flex gap-1 items-center">
                            ...
                            <p className="text-sm font-medium text-gray-700 flex items-center ">
                              {university?.uniCountry ? university?.uniCountry?.countryName?.[language] : "N/A"}
                            </p>
                          </div>
                        ) : (
      ...
      {!hasNextPage && universities.length > 0 && (
        <p className="text-center mt-8 text-gray-500">{t("No More Universities")}</p>
      )}
    </div>
  )
}
